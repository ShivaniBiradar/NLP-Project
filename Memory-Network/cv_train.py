...
## had to import a version of tensorflow that is not the most recent version
## github page says 1.10, but oldest available on pip is 1.13.1 (still works with this version)
...
with open(out_dir+'/params.txt', 'a') as f:
...
## do training and testing over each fold
...
#    E, resolved_scores, essay_id, test_size=.2, random_state=random_state)
    ...
    with open(out_dir+'/params{}.txt'.format(fold_count), 'a') as f:
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=session_conf) as sess:
            ...
            ## go over all training epochs for this fold
            for i in range(1, epochs+1):
                ...
                if i % FLAGS.evaluation_interval == 0 or i == FLAGS.epochs:
                    ...
                    ## use QWK evaluation metric here on training data
                    ...
                    ## use QWK evaluation metric here on testing data
                    ...
                    if(test_kappp_score > best_kappa_so_far):
                        ...
                        # stats on test
                        stat_df.to_csv(out_dir+'/stat.txt')
                        with open(out_dir+'/mem_atten.txt', 'a') as f:
                    ...
                    with open(out_dir+'/eval{}.txt'.format(fold_count), 'a') as f:
